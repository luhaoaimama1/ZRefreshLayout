package zone.com.zrefreshlayout.resistance.utils;
...
public class DampingUtils {
    ...
    public static float damping(float overflowY, float maxLength,float dampingRadio) {
        ...
        //0-1， 指数：1-e;
        double eReplace = linear(dampingRadio,0,1,1, Math.E);
//        double dampingY = -Math.exp(-x *18/maxLength) + 1;
        double dampingY = -Math.pow(eReplace,-x *18/maxLength) + 1;
        return (float) (signum * dampingY)*maxLength;
    }
    public static double linear(double srcNow,double src1,double src2,double dst1,double dst2){
...
//            float value = DampingUtils.damping(i, 1,0.5F);
//            if (value!= 1&&value!=-1)
//                System.out.println("x:" + i +"\t y:"+value+ "\t value:" +value* 300);
//        }

        float map = DampingUtils.damping(108, 4*108,0.5F);
